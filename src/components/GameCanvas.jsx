...
const formatTime = (totalSeconds) => {
  ...
  const s = (totalSeconds % 60).toFixed(0).padStart(2, '0');
  return `${m}:${s}`;
};
...
const LiveHUD = ({ isWin, isPaused, hasStarted, colors, timeRef, wrongMoves, hintsUsed, totalCount }) => {
  ...
  return (
    <div className="ios-glass" style={{
      display: 'flex', flexDirection: 'column', gap: '14px', padding: '16px 24px',
      color: colors.textMain, minWidth: '280px', borderRadius: '20px',
      background: colors.panel, border: `1px solid ${colors.border}`,
      boxShadow: `0 10px 30px rgba(0,0,0,0.4)`, pointerEvents: 'none'
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ color: colors.accent, fontFamily: 'monospace', fontSize: '1.4rem', fontWeight: 800 }}>
          {formatTime(seconds)}
        </span>
      ...
      <div style={{ position: 'relative', marginTop: '4px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: '6px' }}>
          <span style={{ fontSize: '1.2rem', transition: 'all 0.3s ease', color: currentStars >= 1 ? '#ffd700' : 'rgba(255,255,255,0.15)', textShadow: currentStars >= 1 ? '0 0 8px rgba(255,215,0,0.5)' : 'none' }}>★</span>
  ...
  return (
    <div className="confetti-container">
      {confettiPieces.map(p => (
        <div key={p.id} className="confetti-piece" style={{ left: p.left, width: p.size, height: p.size, backgroundColor: p.color, animationDelay: p.delay, animationDuration: p.duration, transform: `rotate(${p.rotation})` }} />
      ))}
    </div>
  );
};
...
const GestureStatus = ({ statusMessage, errorMessage, colors, onDismissError }) => {
  ...
  return (
    <div className="ios-glass gesture-status" style={{ position: 'fixed', bottom: '20px', left: '50%', transform: 'translateX(-50%)', zIndex: 200, padding: '12px 24px', maxWidth: '90vw', textAlign: 'center', display: 'flex', alignItems: 'center', gap: '10px' }}>
      {statusMessage && (
        <><div className="gesture-spinner" /><span style={{ color: colors.accent, fontSize: '0.9rem', fontWeight: 500 }}>{statusMessage}</span></>
      )}
      {errorMessage && (
        <><span style={{ color: '#ff4d4f', fontSize: '0.9rem', fontWeight: 500 }}>⚠️ {errorMessage}</span><button className="ios-btn" onClick={onDismissError} style={{ padding: '4px 12px', fontSize: '0.8rem', marginLeft: '8px' }}>✕</button></>
      )}
    </div>
  );
};
...
export default function GameCanvas({ imgUrl, gridSize, onBack, theme, colors, gestureEnabled = false }) {
  ...
  const handleDismissTutorial = () => { localStorage.setItem('tutorialSeen', 'true'); setShowTutorial(false); };
  const handleHintClick = () => { setShowHint(true); setHintsUsed(prev => prev + 1); setTimeout(() => setShowHint(false), 2500); };
  const showToast = useCallback((msg) => { setGestureToast(msg); setTimeout(() => setGestureToast(null), 1500); }, []);
  const handleDismissError = useCallback(() => { setUseHoloMode(false); }, []);
  ...
  useEffect(() => {
    if (isWin && pieces.length > 0 && !hasSaved) {
      ...
      if (currentTime > totalCount * 20) score -= 2;
      else if (currentTime > totalCount * 10) score -= 1;
      const earnedStars = Math.max(1, score - (hintsUsed > 2 ? 1 : 0));
  ...
  useEffect(() => {
    const calculateLayout = () => {
      ...
      setBoardConfig({ boardW: scaledW, boardH: scaledH, offsetX: boxLeft + (boxW - scaledW) / 2, offsetY: boxTop + (boxH - scaledH) / 2, scale });
      ...
      setPieces(prev => {
        ...
        if (prev.length > 0) {
          ...
          return prev.map((p, index) => {
            ...
            if (p.isFixed) {
              ...
              let startX, startY;
              const stackOffsetX = index * -1.5; const stackOffsetY = index * -1.5;
              if (isMob) {
                const marginY = boxTop + boxH + 20;
                startX = cw / 2 - pieceWidth / 2 + stackOffsetX; startY = marginY + 20 + stackOffsetY;
              } else {
                const marginX = boxLeft + boxW + 20; const marginW = cw - marginX - 20;
                startX = marginX + (marginW - pieceWidth) / 2 + stackOffsetX; startY = ch / 2 - pieceHeight / 2 + stackOffsetY;
              }
        ...
        return shuffled.map((p, index) => {
          ...
          if (isMob) {
            const marginY = boxTop + boxH + 20;
            startX = cw / 2 - p.w / 2 + stackOffsetX; startY = marginY + 20 + stackOffsetY;
          } else {
            const marginX = boxLeft + boxW + 20; const marginW = cw - marginX - 20;
            startX = marginX + (marginW - p.w) / 2 + stackOffsetX; startY = ch / 2 - p.h / 2 + stackOffsetY;
          }
          return { ...p, status: 'board', x: startX - layerOffsetX, y: startY - layerOffsetY };
        });
      });
    };
  ...
  useEffect(() => {
    ...
    if (isClickingUIRef.current) { if (!isPinching) isClickingUIRef.current = false; return; }
    ...
    if (isPinching) {
      if (activePieceRef.current === null) {
        ...
        const pad = 50;
        if (unlocked && unlocked.status === 'board' && canvasX > unlocked.x - pad && canvasX < unlocked.x + unlocked.w + pad && canvasY > unlocked.y - pad && canvasY < unlocked.y + unlocked.h + pad) {
        ...
        if (node) { node.x(canvasX - dragOffsetRef.current.x); node.y(canvasY - dragOffsetRef.current.y); node.getLayer().batchDraw(); }
      }
    } else {
      if (activePieceRef.current !== null) {
        const id = activePieceRef.current;
        ...
        if (piece && calculateDistance(finalX, finalY, piece.correctX, piece.correctY) < snapDistance) {
          ...
          const centerX = finalX + piece.w / 2; const centerY = finalY + piece.h / 2;
          const isOnBoard = centerX >= 0 && centerX <= boardConfig.boardW && centerY >= 0 && centerY <= boardConfig.boardH;
          if (isOnBoard) { setWrongMoves(m => m + 1); isWrongMove = true; }
        }
        ...
        activePieceRef.current = null; setActiveGesturePieceId(null);
      }
    }
  ...
  const handleDragEndKonva = useCallback((e, id) => {
    ...
    if (calculateDistance(finalX, finalY, piece.correctX, piece.correctY) < snapDistance) {
      ...
      const centerX = finalX + piece.w / 2; const centerY = finalY + piece.h / 2;
      const isOnBoard = centerX >= 0 && centerX <= boardConfig.boardW && centerY >= 0 && centerY <= boardConfig.boardH;
      if (isOnBoard) { setWrongMoves(prev => prev + 1); isWrongMove = true; }
    }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, width: '100%', height: '100%', overflow: 'hidden', backgroundColor: colors.bg, touchAction: 'none', WebkitUserSelect: 'none' }} className={`cinematic-enter ${isExiting ? 'page-exit' : ''} ${theme === 'dark' ? 'dark-theme' : 'light-theme'}`}>
      ...
      {useHoloMode && isReady && !isWin && (
        <div style={{ position: 'fixed', top: gestureState.y, left: gestureState.x, width: '24px', height: '24px', backgroundColor: gestureState.isPinching ? '#ff0055' : colors.accent, borderRadius: '50%', transform: 'translate(-50%, -50%)', pointerEvents: 'none', zIndex: 9999, boxShadow: `0 0 ${gestureState.isPinching ? '40px' : '20px'} ${gestureState.isPinching ? '#ff0055' : colors.accent}`, transition: 'background-color 0.15s, box-shadow 0.15s, width 0.15s, height 0.15s', border: '2px solid white' }}>
          <div style={{ position: 'absolute', top: '50%', left: '50%', width: '6px', height: '6px', borderRadius: '50%', backgroundColor: 'white', transform: 'translate(-50%, -50%)' }} />
        </div>
      )}
      <GestureStatus statusMessage={useHoloMode ? statusMessage : ''} errorMessage={useHoloMode ? errorMessage : ''} colors={colors} onDismissError={handleDismissError} />
      {gestureToast && (
        <div className="ios-glass gesture-toast" style={{ position: 'fixed', top: '120px', left: '50%', transform: 'translateX(-50%)', zIndex: 1000, padding: '12px 24px', background: colors.panel, color: colors.accent, borderRadius: '24px', border: `1px solid ${colors.accent}`, boxShadow: `0 0 20px rgba(0,0,0,0.5)`, fontSize: '1.2rem', fontWeight: 'bold', pointerEvents: 'none', animation: 'popInCard 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}>
          ✨ {gestureToast}
        </div>
      )}
      ...
      <div ref={markedAreaRef} style={markedAreaStyle}>
        ...
        <div style={{ position: 'absolute', top: 'max(20px, env(safe-area-inset-top))', left: 'max(20px, env(safe-area-inset-left))', right: 'max(20px, env(safe-area-inset-right))', zIndex: 50, pointerEvents: 'none', display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'center' : 'flex-start', gap: '15px' }}>
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', pointerEvents: 'auto', justifyContent: isMobile ? 'center' : 'flex-start' }}>
            ...
            <button className="ios-btn" onClick={handleHintClick} disabled={showHint || !unlockedPiece} style={{ background: colors.panel, color: colors.textMain, border: `1px solid ${colors.border}`, opacity: (showHint || !unlockedPiece) ? 0.5 : 1 }}>💡 Hint</button>
            <button className="ios-btn gesture-toggle-btn" onClick={handleToggleGesture} style={{ background: useHoloMode ? (isReady ? colors.accent : colors.panel) : colors.panel, color: useHoloMode && isReady ? colors.accentText : colors.textMain, border: `1px solid ${useHoloMode ? colors.accent : colors.border}`, boxShadow: useHoloMode && isReady ? `0 0 20px rgba(0,0,0,0.4)` : 'none', position: 'relative', overflow: 'hidden' }}>
              {useHoloMode && statusMessage && <div style={{ position: 'absolute', bottom: 0, left: 0, height: '3px', background: colors.accent, animation: 'gestureLoadBar 2s ease-in-out infinite', width: '100%' }} />}
              {getGestureButtonLabel()}
            </button>
          </div>

          <LiveHUD isWin={isWin} isPaused={showTutorial} hasStarted={hasStarted} colors={colors} timeRef={timeRef} wrongMoves={wrongMoves} hintsUsed={hintsUsed} totalCount={totalCount} />
        </div>
        ...
        {winPhase === 'card' && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100, pointerEvents: 'auto' }}>
            <div className="ios-glass win-card-animate" style={{ background: colors.panel, padding: '50px 60px', borderRadius: '32px', textAlign: 'center', border: `1px solid ${colors.border}`, boxShadow: `0 30px 60px rgba(0,0,0,0.8)` }}>
              <h1 className="rainbow-text" style={{ fontSize: '3.5rem', margin: '0 0 5px 0', fontWeight: 800 }}>Puzzle Solved</h1>
              ...
              <div style={{ display: 'flex', gap: '20px', justifyContent: 'center' }}>
                <button className="ios-btn highlight-btn" onClick={handleRestart} style={{ fontSize: '1.2rem', padding: '16px 40px', fontWeight: 'bold', boxShadow: `0 10px 25px rgba(0,0,0,0.5)` }}>Play Again</button>
                <button className="ios-btn outline-btn" onClick={handleBack} style={{ fontSize: '1.2rem', padding: '16px 40px', fontWeight: 600 }}>Main Menu</button>
              </div>
            </div>
          </div>
        )}
          ...
          >
            <Layer x={boardConfig.offsetX} y={boardConfig.offsetY}>
              ...
              {boardPieces.filter(p => p.isFixed).map(p => {
                ...
                return (
                  <Shape key={p.id} x={p.x} y={p.y} width={p.w} height={p.h} draggable={false} sceneFunc={(ctx, shape) => { drawPiecePath(ctx, p.w, p.h, p.tabs); ctx.fillStrokeShape(shape); }} fillPatternImage={image} fillPatternScale={{ x: boardConfig.scale, y: boardConfig.scale }} fillPatternOffset={{ x: p.correctX / boardConfig.scale, y: p.correctY / boardConfig.scale }} perfectDrawEnabled={false} listening={false} stroke={isSuccess && showLines ? '#00e676' : colors.accent} strokeWidth={showLines ? (isSuccess ? 4 : 2) : 0} shadowBlur={isSuccess && showLines ? 20 : 0} shadowColor={isSuccess ? '#00e676' : 'transparent'} shadowOpacity={isSuccess && showLines ? 1 : 0} />
                )
              })}
            </Layer>
            <Layer x={boardConfig.offsetX} y={boardConfig.offsetY}>
              {showHint && unlockedPiece && (
                <Shape x={unlockedPiece.correctX} y={unlockedPiece.correctY} width={unlockedPiece.w} height={unlockedPiece.h} sceneFunc={(ctx, shape) => { drawPiecePath(ctx, unlockedPiece.w, unlockedPiece.h, unlockedPiece.tabs); ctx.fillStrokeShape(shape); }} fill="transparent" stroke={colors.accent} strokeWidth={4} shadowBlur={20} shadowColor={colors.accent} shadowOpacity={1} listening={false} />
              )}
              {boardPieces.filter(p => !p.isFixed).slice(0, 2).sort((a, b) => (a.id === unlockedPieceId ? 1 : b.id === unlockedPieceId ? -1 : 0)).map((p) => {
                const isUnlocked = p.id === unlockedPieceId; const isActiveGesture = p.id === activeGesturePieceId; const isError = feedbackStatus?.id === p.id && feedbackStatus?.type === 'error';
                const displayX = isUnlocked ? p.x : p.x + p.w * 0.35; const displayY = isUnlocked ? p.y : p.y + p.h * 0.35;
                ...
                let pieceShadow = isUnlocked ? 15 : 0; let shadowColor = colors.accent;
